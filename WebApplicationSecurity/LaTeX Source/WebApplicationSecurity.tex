...
colback=brown!5!white,colframe=brown!80!gray,height=4.32in,
...
\begin{slide}{Hardware and Software Requirements}{\hyperref[slide 2]{\textless}\hyperref[slide 4]{\textgreater}}
	...
	\begin{instructionblock}
		...
		\begin{enumerate}
			...
			\item {A functional virtualization software platform. For example: VMWare, VirtualBox, or UTM.}
			\item {One vanilla Ubuntu VM (preferably 20.04 LTS) and as an addition, LAMP stack, and \texttt{bWAPP} web application(\textbf{*}).  }
		\end{enumerate}
	\end{instructionblock}
\end{slide}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item `` ` " (single quote) becomes `\&\#039;' (or \&apos;) when ENT\_QUOTES is set. 
		\item `\textless' (less than) becomes `\&lt;' || `\textgreater' (greater than) becomes `\&gt;'
	\end{itemize}		
	}
...
\begin{slide}{Related News: Recent Exploits}{\hyperref[slide 6]{\textless}\hyperref[slide 8]{\textgreater}}
	...
	\begin{instructionblock}
		\begin{enumerate}
			\item {eBay XSS bug left users vulnerable to an (almost) undetectable phishing attack (13 JAN 2016)\cite{ebay-news}}
...
\begin{slide}{Challenge II: Mitigate Challenge I Attack}{\hyperref[slide 13]{\textless}\hyperref[slide 15]{\textgreater}}
	...
	\begin{instructionblock}
		...
		\textit{$/var/www/html/bWAPP/public\_html/xss\_get.php$}\\
...
{Change the security from low to medium or high and repeat the activity. Observe the following changes in the code workflow of the \textbf{csrf.php} file:}
...
A free virtualization software platform - VirtualBox - \href{https://www.virtualbox.org/wiki/Downloads}{\underline{Download}}

A free virtualization software platform for M1 processors - UTM - \href{https://mac.getutm.app/gallery/ubuntu-20-04}{\underline{Download}}{\footnote{{Thanks to Funso Oje (Fall 2021 CptS 427 at Washington State University) for testing the tutorial with 20.04 and for the M1 processor guide link.}}}

Ubuntu 20.04 LTS - Canonical Ltd. - \href{https://releases.ubuntu.com/20.04/}{\underline{Download}} 

Guide to Installing LAMP on Ubuntu 20.04 LTS - DigitalOcean - \href{https://www.digitalocean.com/community/tutorials/how-to-install-linux-apache-mysql-php-lamp-stack-on-ubuntu-20-04}{\underline{Read}}

Guide to Configuring MySQL on Ubuntu 20.04 LTS - DigitalOcean \href{https://www.digitalocean.com/community/tutorials/how-to-install-mysql-on-ubuntu-20-04}{\underline{Read}}{\footnote{{Use this tutorial to setup a root password for your MySQL installation. You will encounter 'connection failed' error if you don't have a root password setup for MySQL.}}}

\texttt{bWAPP} by Malik Mesellem - SourceForge - \href{https://sourceforge.net/projects/bwapp/files/bWAPP/}{\underline{Download}}{\footnote{{If you do not configure a server IP address for Apache, the default address would be `localhost', which can be accessed through `http://localhost'.}}}

{Docker image \texttt{bWAPP} - DockerHub \href{https://hub.docker.com/u/hackersploit}{\underline{Image}}}

{Docker compose \texttt{bWAPP} - GitHub - \href{https://github.com/boxianglin/bWAPP-dockercompose}{\underline{Compose}}}

{Live hosting for \texttt{bWAPP} - HakHub - \href{https://bwapp.hakhub.net/login.php}{\underline{live app}}}
...

{
	\begin{tabular}{ |p{1cm}|p{3cm}|p{3cm}|p{3cm}|  }
		\hline
		\multicolumn{4}{|c|}{Web Application Security Change-log} \\
		\hline
		...
		v 2.3 & Nov. 05th 2021 & Ananth Jillepalli & Updates and solved inconsistencies \\  \hline
		v 2.4 & Nov. 26th 2022 & Ananth Jillepalli & Added docker and live app options for \texttt{bWAPP} hosting \\  \hline
	\end{tabular}
}
